//! Async reliable encoder (sender side).
//!
//! Asynchronous version of [`ReliableEncoder`] for use with async runtimes.

use std::io;
use std::time::Instant;

use crate::streaming::{StreamingEncoder, StreamingParams};
use crate::transport::AsyncDatagramSendMut;
...
/// Async reliable encoder with FEC, ARQ, and congestion control.
///
/// This is the asynchronous version of [`super::ReliableEncoder`], designed for
/// use with async runtimes like Tokio.
///
/// # Example
///
/// ```rust,ignore
/// use cm256::reliable::{AsyncReliableEncoder, ReliableConfig};
...
///
///     let mut encoder = AsyncReliableEncoder::new(ReliableConfig::default(), socket)?;
///     encoder.send(&[0x42; 1200]).await?;
///     Ok(())
/// }
/// ```
#[derive(Debug)]
pub struct AsyncReliableEncoder<T> {
...
impl<T: AsyncDatagramSendMut + Unpin> AsyncReliableEncoder<T> {
    /// Create a new async reliable encoder.
    ...
    /// Process a received ACK packet asynchronously.
    pub async fn on_ack(&mut self, ack: &AckPacket) -> io::Result<()> {
        let now = Instant::now();
    ...
    /// Process a raw received packet (parses ACK if applicable) asynchronously.
    pub async fn process_incoming(&mut self, data: &[u8]) -> io::Result<bool> {
        if data.len() < 3 {
            return Ok(false);
        }
    ...
    /// Retransmit a specific sequence asynchronously.
    ...
    /// Send a source packet asynchronously.
    ...
    /// Send a parity packet asynchronously.
    ...
    /// Flush the FEC encoder asynchronously.
